...
## 7. 红线（最重要）
- **不知道的内容必须先联网搜索核实再写**（百度 / 谷歌 / 官网 / 电商页 / 百科）——WebSearch / WebFetch 查证真实参数、价格、上市日期后再落库，**绝不瞎编**。查不到的字段**留空**走空状态，不要猜。
- specs 尽量带真实来源标注 `["字段","值",["来源"]]`，来源写真实出处，不编来源。
- `hot`/`sales` 是排序信号（相对高低估算），不是真实销量；真实销量/评价放各平台字段并标来源。
